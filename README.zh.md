...
- `--json-content` 一次抓取即可返回元数据、trace 和带外部数据边界的正文。可与 `--output` 同时使用，现有 `--json` 和元数据文件仍不包含正文。
- 丢弃格式损坏的浏览器输出，防止 Cookie 混入正文。JSON 输出和元数据文件中的 URL 凭据也会被遮蔽。
- 当子域名的桌面页面只返回框架外壳时，会额外尝试其 `m.` 地址。
- 仅在需要提取 PDF 时加载 PDF 解析器。
