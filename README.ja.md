...
- `--json-content` は1回の取得でメタデータ、trace、外部データの境界で囲んだ本文を返します。`--output` と併用でき、既存の `--json` とメタデータファイルには本文を含めません。
- 不正なブラウザ出力を破棄し、Cookie が本文に混入する経路を防ぎます。JSON 出力とメタデータファイルの URL 認証情報もマスクします。
- デスクトップ版がフレームセットだけを返すサブドメインでは、`m.` アドレスも試します。
- PDF パーサーは PDF 抽出が必要なときだけ読み込みます。
