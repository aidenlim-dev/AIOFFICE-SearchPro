...
Run manually:
    uv run python skills/aioffice-searchpro/engine/tests/test_url_masking.py
"""
...
def t_trace_masks_credentials_without_changing_request():
    import io
    ...
    url = "https://example.com/?token=TEST_REQUEST_SECRET"
    result = FetchResult(
        ok=True, content="hello", final_url=url,
        trace=[Attempt(phase="probe", executor="curl", url=url,
                       url_transform="original", impersonate=None,
                       referer="https://example.com/?session=TEST_REF_SECRET")],
    )
    stdout, stderr = io.StringIO(), io.StringIO()
    with patch.object(cli, "fetch", return_value=result) as fetch:
        with redirect_stdout(stdout), redirect_stderr(stderr):
            assert cli.main([url, "--trace"]) == 0
    assert fetch.call_args.args[0] == url
    assert "TEST_REQUEST_SECRET" not in stdout.getvalue()
    assert "TEST_REF_SECRET" not in stderr.getvalue()
    assert "session=REDACTED" in stderr.getvalue()
