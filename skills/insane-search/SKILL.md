...
최초 호출 시 OS-native wrapper(`setup/run-engine.sh` 또는 `setup/run-engine.ps1`)가 플러그인 전용 venv(Windows 기본 `%LOCALAPPDATA%\insane-search\venv`, macOS/Linux 기본 `~/.cache/insane-search/venv`, `INSANE_SEARCH_VENV`로 재정의 가능)를 만들고 필요 패키지를 설치한다. 시스템 Python은 건드리지 않는다. **curl_cffi는 0.15.0 이상**을 요구한다 — 0.15부터
