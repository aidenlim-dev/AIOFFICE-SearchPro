...
업스트림 0.16.3까지의 기능, 보안, 성능 개선을 로컬 구조에 맞춰 반영했다.

- 브라우저 출력이 정상 JSON 응답으로 해석되지 않으면 본문을 비우고 실패로 처리한다. 손상된 응답에 담긴 쿠키가 본문과 관측 기록으로 유출되는 경로를 차단한다.
- `m_prefix_subdomain` 변환으로 `sub.example.com`의 `m.sub.example.com` 주소를 탐색한다. 자동, 모바일, 데스크톱 모드별 선택과 프로파일 등록을 함께 반영했다.
- `--json-content`가 한 번의 수집으로 메타데이터, trace, 외부 데이터 경계로 감싼 `untrusted_text`를 반환한다. `--output` 원문 저장과 함께 사용할 수 있으며, 기존 `--json`과 `--metadata` 파일은 본문을 생략한다.
- 로컬 보강: `FetchResult.to_dict()`, `Attempt.to_dict()`, JSON stdout, 메타데이터 파일의 URL 필드에서도 자격 증명을 가린다. 실제 요청 URL과 원문은 유지한다.
- `pypdf`와 선택적 `pdfplumber`를 PDF 추출 시점에 로드한다. 일반 HTML 실행과 크기 제한을 초과한 PDF는 파서를 불러오지 않는다.
- 새 회귀 테스트와 파일 저장 통합 검증을 CI에 추가했다. Claude Code, Codex, marketplace, uv 버전을 `1.7.0`으로 맞췄다.

## 1.6.1 - 2026-09-05

- 업스트림 0.16.1의 URL 자격 증명 마스킹을 반영했다. 추적 로그, 관측 기록, 콘텐츠의 source_url 헤더에서 민감한 쿼리 값과 사용자 인증 정보를 가린다.
- 실제 요청 URL은 유지하며 마스킹 회귀 테스트를 CI에 추가했다.
