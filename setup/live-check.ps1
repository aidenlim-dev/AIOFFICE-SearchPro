...
$Tmp = Join-Path ([IO.Path]::GetTempPath()) ("insane-search-live-" + [Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Force -Path $Tmp | Out-Null

function Get-DefaultVenvDir {
  if ($env:INSANE_SEARCH_VENV) { return $env:INSANE_SEARCH_VENV }
  $localAppData = [Environment]::GetFolderPath("LocalApplicationData")
  if ($localAppData) { return (Join-Path $localAppData "insane-search/venv") }
...
try {
  ...
  if ($env:INSANE_SEARCH_LIVE_EXTENDED -eq "1") {
    $Platforms = @("x", "hn", "arxiv")
    if ($env:INSANE_SEARCH_LIVE_REDDIT -eq "1") { $Platforms += "reddit" }
    if ($env:INSANE_SEARCH_LIVE_MEDIA -eq "1") { $Platforms += "youtube" }
    ...
    $SkillRoot = Join-Path $Root "skills/insane-search"
    if ($OldPythonPath) {
      $env:PYTHONPATH = "$SkillRoot$([IO.Path]::PathSeparator)$OldPythonPath"
    } else {
      $env:PYTHONPATH = $SkillRoot
    }
    & $VenvPython (Join-Path $Root "skills/insane-search/tests/coverage_battery.py") @Platforms
    $Code = $LASTEXITCODE
